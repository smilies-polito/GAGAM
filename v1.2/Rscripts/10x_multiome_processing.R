...
if (!(dir.exists("../TMPResults/classifications/10x_PBMC_Multiome_Controller"))){
  dir.create("../TMPResults/classifications/10x_PBMC_Multiome_Controller")
}

if (!(dir.exists("../TMPResults/GAM"))){
  dir.create("../TMPResults/GAM")
}
if (!(dir.exists("../TMPResults/GAM/10x_PBMC_Multiome_Controller"))){
  dir.create("../TMPResults/GAM/10x_PBMC_Multiome_Controller")
}

if (!(dir.exists("../TMPResults/metrics"))){
  dir.create("../TMPResults/metrics")
}
if (!(dir.exists("../TMPResults/metrics/10x_PBMC_Multiome_Controller"))){
  dir.create("../TMPResults/metrics/10x_PBMC_Multiome_Controller")
}

if (!(dir.exists("../TMPResults/Robjects"))){
  dir.create("../TMPResults/Robjects")
}
if (!(dir.exists("../TMPResults/Robjects/10x_PBMC_Multiome_Controller"))){
  dir.create("../TMPResults/Robjects/10x_PBMC_Multiome_Controller")
}

####### MATRIX LOADING  ########
matrix <- readMM("../TMPDATA/10x_PBMC_Multiome_Controller/matrix.mtx")
cells <- read.table("../TMPDATA/10x_PBMC_Multiome_Controller/barcodes.tsv")
features <- read.delim("../TMPDATA/10x_PBMC_Multiome_Controller/features.tsv", header=FALSE)
...
saveRDS(CDS_RNA,"../TMPResults/Robjects/10x_PBMC_Multiome_Controller/CDS_RNA")
...
#write.table(top_specific_markers_rna$gene_short_name, file='marker_from_RNA_DE_10.tsv', quote=FALSE, sep='\t', col.names = NA)
...
saveRDS(processed_ATAC_cds, "../TMPResults/Robjects/10x_PBMC_Multiome_Controller/processed_ATAC_cds")
...
saveRDS(connection_table, "../TMPResults/Robjects/10x_PBMC_Multiome_Controller/connection_table")
#connection_table <- readRDS("../TMPResults/connection_table_10k_Multiome")
...
####### LABELING PEAKS ###########
labeled_peaks <- read.csv("../TMPresults/labeled_peaks/10x_PBMC_Multiome_Controller/cCRE_labeled_peaks.tsv", sep = "\t")
...
saveRDS(labeled_peaks, "../TMPResults/Robjects/10x_PBMC_Multiome_Controller/labeled_peaks")
saveRDS(nmax, "../TMPResults/Robjects/10x_PBMC_Multiome_Controller/nmax")
...
saveRDS(PBMC, "../TMPResults/Robjects/10x_PBMC_Multiome_Controller/PBMC")
...
cds_cicero <- preprocess_cds(cds_cicero, method = "LSI")
...
write.table(c_gam, file='../TMPResults/GAM/10x_PBMC_Multiome_Controller/cicero.tsv', quote=FALSE, sep='\t', col.names = NA)
write.table(class2, file='../TMPResults/classifications/10x_PBMC_Multiome_Controller/cicero_classification.tsv', quote=FALSE, sep='\t', col.names = NA)
write.table(class, file='../TMPResults/classifications/10x_PBMC_Multiome_Controller/atac_classification.tsv', quote=FALSE, sep='\t', col.names = NA)
...
chr2acc <- read.csv("../TMPResults/Gene_2022/Genomes/hg38/chr2acc.txt", sep = "\t")
gene_anno <- rtracklayer::readGFF("../TMPResults/Gene_2022/Genomes/hg38/GCF_000001405.39_GRCh38.p13_genomic.gtf.gz")
gene_anno <- gene_anno[gene_anno$seqid %in% chr2acc$Accession.version,]
gene_anno$seqid <- as.factor(as.character(gene_anno$seqid))
levels(gene_anno$seqid) <- chr2acc$X.Chromosome
gene_anno$seqid <- paste0("chr", gene_anno$seqid)
...
write.table(tsv, file='../TMPResults/GAM/10x_PBMC_Multiome_Controller/gene_scoring.tsv', quote=FALSE, sep='\t', col.names = NA)
write.table(class3, file='../TMPResults/classifications/10x_PBMC_Multiome_Controller/genescoring_classification.tsv', quote=FALSE, sep='\t', col.names = NA)
