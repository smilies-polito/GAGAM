...
#  matrix.txt - > scATAC-seq matrix
#  cell.txt -> cells
#  conversion_error.txt -> conversion error
#
...
cells <- cells$V1
#peaks <- paste0(buenrostro_peaks$V1,"_",buenrostro_peaks$V2, "_", buenrostro_peaks$V3)
...
saveRDS(conns, "../TMPResults/conns_buenrostro")
...
peaks_multi_info <- cbind(peaks_multi_info,labeled_peaks[7:26])
...
intragenetic_non_prom_peaks <- ClosestFeature(PBMC, gsub("_", "-", non_prom_list), refseq_GRanges)
...
if ("dist" %in% colnames(conns_buenrostro) == FALSE) {
  Peak1_cols <- split_peak_names(conns_buenrostro$Peak1)
  Peak2_cols <- split_peak_names(conns_buenrostro$Peak2)
  ...
  conns_buenrostro$dist <- abs(Peak2_bp - Peak1_bp)
}
...
prom_enhD <- conns_buenrostro[(conns_buenrostro$Peak1 %in%
                            promoter_peak_table$peak &
                              conns_buenrostro$Peak2 %in%
                            enhD_peaks_list) | (conns_buenrostro$Peak2 %in%
                                                  promoter_peak_table$peak &
                                                  conns_buenrostro$Peak1 %in%
                                                  enhD_peaks_list),]
...
prova <- gene_activity_scores #* refseq_first_gene_matrix


#final_GAM <- prova + refseq_allgene_gene_matrix
final_GAM <- gene_activity_scores #+ refseq_first_gene_matrix2
...
final_GAM_cds <- reduce_dimension(final_GAM_cds, reduction_method = 'UMAP', 
                                     preprocess_method = "PCA")
final_GAM_cds = cluster_cells(final_GAM_cds, resolution=5e-3)

#_cells(final_GAM_cds, cell_size = 1.2, group_label_size = 5)


final_GAM_cds@colData@listData[["label"]] <- metadata$label
...
refseq_promoter_gene_mat <- refseq_promoter_gene_mat[refseq_near_prom_list,]
refseq_accessibility_mat <- exprs(input_cds)
refseq_promoter_activity_scores <- refseq_accessibility_mat[refseq_near_prom_list,, drop=FALSE]
